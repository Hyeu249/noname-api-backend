    ...
    try {
      for (const key of structKeys) {
        ...
        if (struct[key].validate.find((v) => v.includes("required"))) {
          if (req[key] === undefined) throw new Error(domain.RequiredFieldError);
        }
        ...
        if (struct[key].validate.find((v) => v.includes("gte"))) {
          ...
          if (isfieldHaveValue) {
            if (req[key] < gte.split("=")[1]) throw new Error(domain.GteValidationFailed);
          }
        }

        //validate email
        if (struct[key].validate.find((v) => v.includes("email"))) {
          if (!isEmail(String(req[key]))) throw new Error(domain.PayloadEmailInvalid);
        }
      }
    } catch (error) {
      this.#error = error;
  ...
  Parse() {
    ...
    try {
      ...
      switch (parseError) {
        ...
        case domain.PayloadEmailInvalid:
          return [null, domain.PayloadEmailInvalid];
        default:
