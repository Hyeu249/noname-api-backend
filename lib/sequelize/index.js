...
function CreateSchema(sequelizeDb) {
  try {
    ...
    //one to many for image and product type
    ...
    //one to one for image and product
    Image.Modal.hasMany(Product.Modal);
    Product.Modal.belongsTo(Image.Modal, {
      foreignKey: {
        allowNull: false,
        name: "image_id",
      },
    ...
    //one to one for product_type and product
    ProductType.Modal.hasMany(Product.Modal);
    Product.Modal.belongsTo(ProductType.Modal, {
      foreignKey: {
        allowNull: false,
