...
function CreateSchema(sequelizeDb) {
  try {
    ...
    var [err] = ProductCollectionRelation.Schema.Init(sequelizeDb, getProductCollection());
    if (err !== null) {
      throw new Error(err);
    }

    User.Relation.Init(modals);
    Image.Relation.Init(modals);
...
function getProductCollection() {
  return { Product: Product.Modal, Collection: Collection.Modal };
}
